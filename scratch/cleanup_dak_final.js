...
doc += '| **Reviewed By** | TBD (Head of Agronomy & Nursery Technical Specialist) |\n';
doc += '| **Approved By** | TBD (Head of Plantation & IT Project Steering Committee) |\n\n';
...
doc += '| **v1.0.0** | 2026-09-07 | System Architect | **FINAL / FOR REVIEW** | **Finalisasi 172 Active Requirements, 18 Business Rules, 175 Flow Nodes, 156 Edges, 5 CFE, dan 100% RTM Traceability.** |\n\n';
...
doc += 'Dokumen ini mendefinisikan secara ketat **7 Peran Master Pengguna** berdasarkan kewenangan yang termaktub pada baseline requirement:\n\n';
...
// M. DATA REQUIREMENTS
doc += '\n# M. DATA REQUIREMENTS\n\n';
doc += 'Struktur entitas data yang dikelola dalam SIGMA Rubber Nursery mengacu pada entitas domain baseline:\n';
...
doc += '> *Catatan:* Detail skema field atribut level database yang belum tercantum secara eksplisit pada baseline requirement akan didefinisikan pada tahap Software Design Description (SDD).\n';

// N. VALIDATION & BUSINESS LOGIC
doc += '\n# N. VALIDATION & BUSINESS LOGIC\n\n';
doc += 'Katalog validasi operasional yang didukung langsung oleh baseline requirement dan business rules kanonikal:\n';
doc += '* **Geofencing GPS:** Memastikan operator berada di dalam radius <= 500 meter dari titik sentroid bibitan/kebun entres saat melakukan presensi dan transaksi penting (`BR-PRS-003`, `RN-PRS-006`).\n';
doc += '* **Scan QR Code:** Validasi QR plang batch polybag dan bedengan memastikan tidak salah plot/varietas sebelum memulai okulasi atau perawatan (`RN-OKL-002`, `RN-SEL-003`).\n';
doc += '* **Quantity & Balance Ceiling:** Transaksi pemakaian benih, kayu entres, pupuk, dan muat bibit tidak boleh melebihi sisa saldo populasi aktif atau kuota BKB gudang (`BR-MAT-001`, `BR-SEM-001`).\n';
doc += '* **Segregasi Otorisasi:** Operator yang mengusulkan mutasi (Mantri) dilarang melakukan self-approval; approval wajib dieksekusi oleh Asisten Bibitan / Askep / Pengurus (`BR-GLB-003`, `BR-SEL-001`).\n';
...
// O. STATUS & WORKFLOW
doc += '\n# O. STATUS & WORKFLOW\n\n';
doc += 'Siklus status dokumen dan transaksi operasional mengikuti transisi status aktual pada baseline data:\n';
...
doc += '\n# P. AUDIT TRAIL & CORRECTION GOVERNANCE\n\n';
doc += 'Berdasarkan requirement **`RN-MNT-009`** dan aturan tata kelola **`BR-AUD-001`**, setiap perbaikan atau koreksi terhadap transaksi yang berstatus *Confirmed / Approved* wajib merekam 5 parameter audit mandatory:\n';
...
doc += 'Tata kelola audit trail ini bersifat lintas-modul (*cross-cutting governance*) dan berlaku untuk seluruh transaksi confirmed.\n';
...
doc += 'Laporan yang tersedia dalam sistem mengikuti requirement fungsional dan fitur pelaporan yang didefinisikan pada baseline:\n';
doc += '1. **Laporan Keterlacakan Persyaratan (DOC-04):** Matriks keterlacakan 172 requirement ke flow node dan business rule.\n';
doc += '2. **Laporan Analisis Kesenjangan & Kesiapan Sistem (DOC-05):** Pemantauan status coverage alur proses (Zero Gap status).\n';
doc += '3. **Laporan Transaksional Modul Lapangan:** Rekapitulasi presensi pekerja, kartu seleksi bibit, dan rekap heading kerja pemeliharaan.\n\n';
doc += '> *Catatan:* Format dan visualisasi laporan analitik lanjutan lainnya akan diselaraskan pada modul pelaporan operasional.\n';

// T. SECURITY & ACCESS CONTROL
doc += '\n# T. SECURITY & ACCESS CONTROL\n\n';
doc += 'Kontrol keamanan dan akses sistem berakar langsung dari pembagian wewenang peran pada baseline requirement:\n';
doc += '* **Role-Based Access Control (RBAC):** Hak akses menu, input transaksi, dan verifikasi dibatasi secara ketat berdasarkan 7 Peran Master.\n';
doc += '* **Segregasi Otorisasi:** Pemisahan tegas antara peran operator pembuat transaksi (Mantri) dan peran pemberi persetujuan (Asisten/Askep/Pengurus).\n';
doc += '* **Geofence Security:** Validasi geolokasi GPS memastikan transaksi hanya dapat dieksekusi di dalam perimeter resmi pembibitan (`BR-PRS-003`).\n';
doc += '* **Integritas Transaksi:** Transaksi berstatus Confirmed dikunci dari perubahan langsung dan wajib melalui jalur audit trail koreksi (`BR-AUD-001`).\n\n';
doc += '> *Catatan Arsitektur:* Mekanisme teknis otentikasi protokol (seperti token sesi atau integrasi identity provider) belum ditentukan secara spesifik pada baseline requirement fungsional dan akan ditetapkan pada tahap Software Design Description (SDD).\n';

// U. OFFLINE CAPABILITY & DATA SYNCHRONIZATION
doc += '\n# U. OFFLINE CAPABILITY & DATA SYNCHRONIZATION\n\n';
doc += 'Kebutuhan operasional mendukung penggunaan sistem pada kondisi konektivitas terbatas/offline sesuai kebutuhan lapangan di areal pembibitan:\n';
doc += '* **Dukungan Operasional Offline:** Pengguna dapat melakukan entri data transaksi lapangan saat tidak tersedia koneksi internet.\n';
doc += '* **Prinsip Sinkronisasi:** Data transaksi lokal akan disinkronisasikan ke server pusat saat konektivitas kembali tersedia.\n\n';
doc += '> *Catatan Arsitektur:* Detail mekanisme penyimpanan data lokal (local cache storage), antrean pengiriman data, dan algoritma resolusi sinkronisasi ditetapkan pada tahap Software Design / implementasi teknis aplikasi mobile.\n';

// V. NON-FUNCTIONAL REQUIREMENTS
doc += '\n# V. NON-FUNCTIONAL REQUIREMENTS\n\n';
doc += 'Prinsip-prinsip non-fungsional sistem dirumuskan sebagai panduan arsitektural:\n';
doc += '* **Performa & Waktu Respon:** Parameter performa sistem belum ditetapkan secara numerik pada baseline requirement dan perlu ditentukan pada tahap Software Design / SLA.\n';
doc += '* **Ketersediaan Sistem (Availability):** Target availability sistem belum ditetapkan secara numerik pada baseline requirement dan perlu ditentukan pada dokumen SLA Infrastruktur/Hosting.\n';
doc += '* **Kemudahan Penggunaan (Usability):** Antarmuka ramah lapangan dengan kontras visual memadai, ukuran tombol yang proporsional untuk operasional luar ruangan, serta alur navigasi intuitif.\n';
doc += '* **Kompatibilitas:** Aplikasi mobile dirancang agar kompatibel dengan peramban modern pada perangkat smartphone standar yang digunakan di perkebunan.\n';

// W. ASSUMPTIONS & LIMITATIONS
doc += '\n# W. ASSUMPTIONS & LIMITATIONS\n\n';
doc += 'Asumsi dan batasan operasional yang didukung langsung oleh baseline requirement dan aturan bisnis:\n';
doc += '* **Perangkat Lapangan:** Pengguna operasional (Mantri Bibitan) diasumsikan menggunakan perangkat smartphone yang dilengkapi modul GPS aktif dan kamera untuk pengambilan dokumentasi geotagged (`BR-GLB-001`, `RN-PRS-006`).\n';
doc += '* **Standar Jam Kerja:** Perhitungan mandays tenaga kerja mengacu pada ketentuan operasional 7 jam kerja reguler dan 5 jam kerja hari Jumat sesuai formula kalkulasi output pekerjaan (`RN-MNT-004`).\n';
doc += '* **Kemurnian Klon:** Setiap bedengan atau batch bibit polybag diasumsikan homogen (satu klon murni per batch) sesuai tata kelola pembibitan karet Socfindo (`BR-OKL-002`).\n';
...
console.log('Cleaned TASK-13-DAK-FINAL.md written successfully!');
