...
export function renderSeedingForm() {
  ...
  app.innerHTML = `
    <div class="page" style="display: flex; flex-direction: column; height: 100%; background: #FFFFFF; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;">
      ...
      <header style="display: flex; align-items: center; height: 50px; padding: 0 14px; background: #FFFFFF; border-bottom: 1px solid #E5E7EB; flex-shrink: 0;">
        ...
        <h1 style="font-size: 0.95rem; font-weight: 700; color: #111827; margin: 0 0 0 8px;">Penyemaian</h1>
      </header>
      ...
      <main style="flex: 1; overflow-y: auto; padding-bottom: 20px;">
        ...
        <section style="padding: 12px 14px; border-bottom: 1px solid #E5E7EB; background: #FFFFFF;">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
            <h2 style="font-size: 0.82rem; font-weight: 700; color: #111827; margin: 0;">Detail Penyemaian</h2>
            <button id="btn-tambah-data" type="button" style="background: #116834; color: white; border: none; border-radius: 4px; padding: 4px 8px; font-size: 0.70rem; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 4px;">
          ...
          <!-- Table Header -->
          <div style="background: #F9FAFB; padding: 6px 4px; display: grid; grid-template-columns: 1.3fr 1.2fr 1fr 24px; gap: 4px; border: 1px solid #E5E7EB; border-bottom: none; align-items: center; text-align: center;">
            <div style="font-size: 0.66rem; font-weight: 700; color: #4B5563; text-align: left; padding-left: 4px;">No. Batch</div>
            <div style="font-size: 0.66rem; font-weight: 700; color: #4B5563;">Bibit Pindah Semai</div>
            <div style="font-size: 0.66rem; font-weight: 700; color: #4B5563;">Jlh Polybag</div>
            <div></div>
          </div>
          
          <div id="table-body" style="border: 1px solid #E5E7EB; border-top: none;">
            <!-- rows -->
          </div>

          <!-- Banyaknya Ditolak / Seleksi (Simetris 2 Kolom) -->
          <div style="margin-top: 10px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; align-items: center;">
              <input type="number" id="input-ditolak" value="${state.ditolak}" placeholder="0" style="width: 100%; height: 32px; border: 1px solid #D1D5DB; border-radius: 4px; padding: 0 8px; text-align: left; font-size: 0.76rem; font-weight: 600; color: #111827; background: #FFFFFF; outline: none; box-sizing: border-box;">
              <div style="position: relative; width: 100%; height: 32px;">
                <select id="select-alasan" style="width: 100%; height: 32px; border: 1px solid #D1D5DB; border-radius: 4px; background: #FFFFFF; padding: 0 6px; font-size: 0.74rem; font-weight: 500; color: #374151; cursor: pointer; outline: none; box-sizing: border-box;" ${(!state.ditolak || parseInt(state.ditolak) === 0) ? 'disabled' : ''}>
  ...
  function validateForm() {
    ...
    state.tableRows.forEach(row => {
      const disVal = parseInt(row.disemai || 0);
      disemaiTotal += disVal;
      if ((!row.batchNo && !row.batchId) || disVal <= 0) {
        isValid = false;
      }
    });

    // Validasi tidak boleh melebihi Jlh Berhasil di Deder
    if (disemaiTotal > totalPenerimaan) {
      isValid = false;
    }
  ...
  function renderTableRows() {
    tableBody.innerHTML = state.tableRows.map((row, idx) => `
      <div class="table-row" style="display: grid; grid-template-columns: 1.3fr 1.2fr 1fr 24px; gap: 4px; padding: 4px; border-bottom: 1px solid #E5E7EB; align-items: center; background: #FFFFFF;">
        <select class="sel-batch" data-index="${idx}" style="width: 100%; height: 32px; border: 1px solid #D1D5DB; border-radius: 4px; outline: none; background: #FFFFFF; font-size: 0.74rem; font-weight: 600; color: #111827; cursor: pointer; padding: 0 4px; box-sizing: border-box;">
          ${finalBatchList.map(b => `<option value="${b.id || b.batchId}" ${(row.batchId === (b.id || b.batchId) || row.batchNo === (b.batchCode || b.batchNo)) ? 'selected' : ''}>${b.batchCode || b.batchNo}</option>`).join('')}
        </select>
        <input type="number" class="inp-disemai" data-index="${idx}" value="${row.disemai}" placeholder="0" max="${totalPenerimaan}" style="width: 100%; height: 32px; border: 1px solid #D1D5DB; border-radius: 4px; outline: none; font-size: 0.76rem; font-weight: 600; text-align: center; background: #FFFFFF; color: #111827; padding: 0 4px; box-sizing: border-box;">
        <input type="number" class="inp-polybag" data-index="${idx}" value="${row.polybag}" placeholder="0" readonly style="width: 100%; height: 32px; border: 1px solid #E5E7EB; border-radius: 4px; outline: none; font-size: 0.76rem; font-weight: 600; text-align: center; background: #F9FAFB; color: #4B5563; padding: 0 4px; box-sizing: border-box;">
        <div style="display: flex; justify-content: center; align-items: center;">
          ${state.tableRows.length > 1 ? `
            <button type="button" class="btn-hapus-row" data-index="${idx}" title="Hapus baris" style="background: transparent; border: none; width: 22px; height: 22px; padding: 0; cursor: pointer; color: #DC2626; display: flex; align-items: center; justify-content: center;">
          ...
          ` : `
            <button type="button" class="btn-reset-row" data-index="${idx}" title="Kosongkan baris" style="background: transparent; border: none; width: 22px; height: 22px; padding: 0; cursor: pointer; color: #9CA3AF; display: flex; align-items: center; justify-content: center;">
              <svg viewBox="0 0 24 24" width="13" height="13" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
                <line x1="18" y1="6" x2="6" y2="18"></line>
                <line x1="6" y1="6" x2="18" y2="18"></line>
              </svg>
    ...
    tableBody.querySelectorAll('.sel-batch').forEach(el => {
      el.addEventListener('change', (e) => {
        ...
        if (state.tableRows[rowIdx]) {
          state.tableRows[rowIdx].batchId = bObj ? (bObj.id || bObj.batchId) : chosenId;
          state.tableRows[rowIdx].batchNo = bObj ? (bObj.batchCode || bObj.batchNo) : chosenId;
        }
        validateForm();
      });
    });
    ...
    tableBody.querySelectorAll('.inp-disemai').forEach(el => {
      el.addEventListener('input', (e) => {
        ...
        // Validasi: Bibit di Pindah Semai tidak boleh melebihi Jlh Berhasil di Deder
        const maxLimit = totalPenerimaan;
        if (val > maxLimit) {
          toast(`Bibit di Pindah Semai tidak boleh melebihi Jlh Berhasil di Deder (${maxLimit.toLocaleString('id-ID')} Butir).`, 'error');
          val = maxLimit;
  ...
  btnKonfirmSimpan.addEventListener('click', () => {
    ...
    if (totalDisemai > totalPenerimaan) {
      toast(`Bibit di Pindah Semai (${totalDisemai.toLocaleString('id-ID')} Butir) tidak boleh melebihi Jlh Berhasil di Deder (${totalPenerimaan.toLocaleString('id-ID')} Butir).`, 'error');
      return;
    }
