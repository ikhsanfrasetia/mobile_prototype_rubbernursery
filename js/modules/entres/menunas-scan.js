/**
 * modules/entres/menunas-scan.js — Fitur Scan QR Code Plot Entres untuk Menunas.
 * Mengadopsi pola dan metode persis seperti fitur scan QR Bedengan di Penyemaian.
 * - Terbuka otomatis saat masuk ke #/entres/menunas
 ...
 * - Navigasi langsung ke #/entres/menunas/form
 */
...
export function renderMenunasScan() {
    ...
    <div class="page menunas-scan-page" style="display: flex; flex-direction: column; height: 100%; background: #0F172A; color: #FFFFFF; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; position: relative; overflow: hidden;">
      ...
      <header style="display: flex; align-items: center; justify-content: space-between; height: 56px; padding: 0 16px; background: rgba(15, 23, 42, 0.85); backdrop-filter: blur(8px); border-bottom: 1px solid rgba(255,255,255,0.1); z-index: 10; flex-shrink: 0;">
        ...
        <div style="text-align: center; flex: 1; padding: 0 8px;">
          ...
          <div style="font-size: 0.68rem; color: #94A3B8; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">Kegiatan Menunas Entres</div>
        </div>
      ...
      <main style="flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 16px 16px 20px; position: relative; z-index: 5;">
        ...
        <div id="box-scan-failed" style="display: none; width: 100%; max-width: 330px; background: #FEF2F2; border: 1px solid #FECACA; border-radius: 8px; padding: 12px 14px; margin-bottom: 8px; text-align: center; box-sizing: border-box; animation: fadeIn 0.3s ease;">
          <div style="font-size: 0.82rem; font-weight: 800; color: #991B1B; margin-bottom: 2px;">
            ⚠️ Gagal Memindai QR Plot Entres
          </div>
          <p style="font-size: 0.72rem; color: #7F1D1D; margin: 0 0 10px; line-height: 1.35;">
            QR Code tidak terbaca atau belum terpasang di patok plot. Silakan pilih plot dari daftar manual.
          </p>
          <button id="btn-pilih-manual" type="button" style="width: 100%; height: 42px; background: #DC2626; color: #FFFFFF; border: none; border-radius: 6px; font-weight: 800; font-size: 0.84rem; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; box-shadow: 0 2px 8px rgba(220,38,38,0.3);">
            ...
            <span>Pilih Plot Manual</span>
          </button>
        </div>

      </main>

      <!-- BOTTOM SHEET: PILIH PLOT ENTRES MANUAL (PERSIS POLA PENYEMAIAN) -->
      ...
      <div id="sheet-manual-plot" style="display: none; position: absolute; left: 0; right: 0; bottom: 0; background: #FFFFFF; color: #111827; border-radius: 18px 18px 0 0; padding: 20px 16px 24px; z-index: 51; flex-direction: column; max-height: 75vh; box-shadow: 0 -8px 24px rgba(0,0,0,0.3); transition: transform 0.25s ease;">
  ...
  const proceedWithPlot = (plotData, verifiedMethod = 'QR_SCAN') => {
    ...
    // Simpan ke storage untuk digunakan di menunas-form.js
    storage.set('selected_menunas_plot', plotData);
    ...
    // Navigasi langsung ke Form Transaksi Menunas
