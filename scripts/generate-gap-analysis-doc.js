...
const data = JSON.parse(fs.readFileSync('scripts/gap_analysis_data.json', 'utf8'));
const { coverageMetrics, gapReport, detailedGaps, gapsByModule, legacyContamination, crossFlowEdges } = data;

const doc = `# POST-CLEANUP GAP ANALYSIS VALIDATION REPORT
...
|:---:|---|---|---|---|:---:|:---:|---|
${detailedGaps.map((g, idx) => `| ${idx + 1} | \`${g.id}\` | ${g.role} | ${g.module} | ${g.feature} | ${g.reqStatus} | ${g.businessRule} | ${g.reason} |`).join('\n')}
...
fs.writeFileSync('POST_CLEANUP_GAP_ANALYSIS_AUDIT.md', doc, 'utf8');
console.log('✅ POST_CLEANUP_GAP_ANALYSIS_AUDIT.md generated successfully.');
